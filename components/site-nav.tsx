    ...
    <header className="site-header fixed inset-x-0 top-0 z-50 border-b border-[var(--line)] bg-[var(--surface)]/92 backdrop-blur">
      <nav aria-label="Primary navigation" className="site-nav-inner mx-auto flex min-h-[72px] w-full max-w-6xl items-center justify-between gap-3 px-5 lg:px-8">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <ButtonLink href={site.github} variant="ghost" className="hidden px-3 lg:inline-flex" icon={<GitBranch size={16} />} eventName="GitHub Visit">GitHub</ButtonLink>
          <ButtonLink href={site.emailHref} variant="secondary" className="px-3 max-sm:px-2" icon={<Mail size={16} />} eventName="Contact">
            <span className="max-sm:sr-only">Email</span>
          </ButtonLink>
          <ButtonLink href={site.resume} variant="primary" className="px-3 max-sm:px-2" icon={<Download size={16} />} eventName="Resume Download">
            <span className="max-sm:sr-only">Resume</span>
          </ButtonLink>
          <button
            type="button"
            className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg border border-[var(--line)] text-[var(--ink)] transition hover:border-[var(--accent)] lg:hidden"
      ...
      <div
        ...
        className={`mobile-navigation border-t border-[var(--line)] bg-[var(--surface)] px-5 py-4 lg:hidden ${menuOpen ? "block" : "hidden"}`}
      >
