<template>
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        <div class="flex flex-col h-48" v-for="repo in repos.values" :key="repo.id">
            <a target="_blank" :href="repo.website ? repo.website : repo.links.html.href" class="bg-white rounded-lg shadow-sm hover:shadow-md p-4 flex flex-col justify-between leading-normal flex flex-col flex-1">
                <Card :repo="repo" />
